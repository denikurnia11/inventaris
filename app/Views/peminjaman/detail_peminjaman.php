...
<div class="row">
  <div class="col-lg-12"><br />
    <ol class="breadcrumb">
      <li><a href="<?= base_url('admin/peminjaman/daftar'); ?>">Daftar Peminjaman</a></li>
        ...
        <form class="form-horizontal">
          ...
          <div style="display: flex; width: 100%; justify-content: center;">
            <?php if ($peminjaman['status'] === 'pending') : ?>
              <button type="button" class="btn btn-success btn-sm" onclick="setuju()">Setuju</button>
              <button type="button" class="btn btn-danger btn-sm ml-2" onclick="tolak()">Tolak</button>
            <?php elseif ($peminjaman['status'] === 'dipinjam') : ?>
              <button type="button" class="btn btn-warning btn-sm ml-2" onclick="kembali()">Kembali</button>
            <?php endif ?>
          </div>
        </form>
      </div>
...
<script type="text/javascript">
  ...
  const setuju = () => {
    $.ajax({
      url: `<?= base_url() ?>/api/peminjaman/${id_peminjaman}/setuju`,
      method: 'GET',
      error: (data) => {
        $('#alert').html(`<div class="alert alert-danger">${data.responseJSON.error}</div>`)
      },
      success: (data) => {
        window.location.replace('<?= base_url('admin/peminjaman') ?>')
      }
    })
  }
  ...
  const tolak = () => {
    $.ajax({
      url: `<?= base_url() ?>/api/peminjaman/${id_peminjaman}/tolak`,
      method: 'GET',
      error: (data) => {
        $('#alert').html(`<div class="alert alert-danger">${data.responseJSON.error}</div>`)
      },
  ...
  const kembali = () => {
    $.ajax({
      url: `<?= base_url() ?>/api/peminjaman/${id_peminjaman}/kembali`,
      method: 'GET',
      error: (data) => {
        $('#alert').html(`<div class="alert alert-danger">${data.responseJSON.error}</div>`)
      },
